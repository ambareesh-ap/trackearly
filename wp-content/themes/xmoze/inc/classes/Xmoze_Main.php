...
namespace XmozeTheme\Inc\Classes;
...
class Xmoze_Main
{
    ...
    public function postMarkupGenerator($layout, $grid)
    {
        ...
        if (!is_active_sidebar('xmoze_blog_sidebar')) :
            ...
            if (isset($layout)) :
                ...
                elseif ($layout == 'left-sidebar') :
                    ...
                    $this->getPulledSidebar('col-lg-4 col-md-10 left-sidebar');
                    $this->thePostLoop('col-md-8 ' . $grid, $grid);

                elseif ($layout == 'right-sidebar') :

                    $this->thePostLoop('col-lg-8 col-md-10 ' . $grid, $grid);
                    $this->getPulledSidebar('col-lg-4 col-md-10 right-sidebar');
                endif;

            else :

                $this->thePostLoop('col-lg-8 col-md-10', $grid);
                $this->getPulledSidebar('col-lg-4 col-md-10 right-sidebar col-lg-offset-1');
    ...
    public function pagination()
    {
        ...
        echo '<div class="col-12"><div class="xmoze-navigation">';
    ...
    public function xmoze_theContentWithComment()
    {
    ...
    public function xmoze_breadcrumb_bridge()
    {
        $xmoze = get_option('xmoze');

        if (isset($xmoze['breadcrumb_on'])) :
            if ($xmoze['breadcrumb_on'] == true) :
                $this->xmoze_get_the_breadcrumbs();
            endif;
        else :
            $this->xmoze_get_the_breadcrumbs();
        endif;
     ...
     * @return breadcrumb
     */
    public function xmoze_get_the_breadcrumbs()
    {
        $xmoze = get_option('xmoze');
    ...
    public function generateBreadCrumbTitle()
    {
        ...
        if (is_home() || is_front_page()) {
            $title = isset($xmoze['bp_title']) ? esc_html($xmoze['bp_title']) : esc_html__('Our Blog', 'xmoze');
        } elseif ('case-study' == xmoze_get_archive_post_type()) {
            $title = isset($xmoze['cs_title']) ? esc_html($xmoze['cs_title']) : esc_html__('Case Study', 'xmoze');
        } elseif ('job' == xmoze_get_archive_post_type()) {
            $title = isset($xmoze['job_title']) ? esc_html($xmoze['job_title']) : esc_html__(' Current job openings', 'xmoze');
        } elseif (is_page()) {
            $title = get_the_title();
        } elseif (is_single()) {
            $title = get_the_title();
        }elseif (function_exists('is_shop') && is_shop()) {
            $title = isset($xmoze['shop_title']) ? esc_html($xmoze['shop_title']) : esc_html__('Our Products', 'xmoze');
        } elseif (is_archive()) {
            $title = get_the_archive_title();
        } elseif (is_search()) {
            $title = esc_html__('Search results for: ', 'xmoze') . get_search_query();
        }elseif(is_404(  )){
            $title = isset($xmoze['404_title']) ? esc_html($xmoze['shop_title']) : esc_html__('404', 'xmoze');
        }

        return $title;
    }

    public static function xmoze_breadcrumbs()
    {

        $xmoze = get_option('xmoze');

        $sepOpt = (isset($xmoze['breadcrumb_sep']) ? $xmoze['breadcrumb_sep'] : '<i class="fa fa-angle-right"></i>');

        /* === OPTIONS === */
        $text['home'] = esc_html__('Home', 'xmoze'); // text for the 'Home' link
        $text['shop'] = esc_html__('Shop', 'xmoze'); // text for the 'Shop' link
        $text['category'] = esc_html__('Archive by Category "%s"', 'xmoze'); // text for a category page
        $text['search'] = esc_html__('Search Results for "%s" Query', 'xmoze'); // text for a search results page
        $text['tag'] = esc_html__('Posts Tagged "%s"', 'xmoze'); // text for a tag page
        $text['author'] = esc_html__('Articles Posted by %s', 'xmoze'); // text for an author page
        $text['404'] = esc_html__('Error 404', 'xmoze'); // text for the 404 page
        $text['page'] = esc_html__('Page %s', 'xmoze'); // text 'Page N'
        $text['cpage'] = esc_html__('Comment Page %s', 'xmoze'); // text 'Comment Page N'
        ...
        if (is_home() && is_front_page()) {
            ...
            if (is_category()) {
                ...
                if (get_post_type() != 'post') {
                    ...
                    $post_type = get_post_type_object(get_post_type());
                    $slug = 'post' == get_post_type(  ) ? get_the_title() : $post_type->rewrite ;
